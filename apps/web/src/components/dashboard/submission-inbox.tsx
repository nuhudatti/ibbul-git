    ...
    <div className="flex flex-col h-full min-h-0">
      <div className="shrink-0 px-5 py-4 border-b border-white/6">
        ...
        <p className="text-[11px] text-zinc-500 mt-1">
          Real work from your class — appears when students hit Submit in the IDE
        </p>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2">
            {filtered.map((row, i) => {
              ...
              const pending = artifact && !artifact.verified;
              return (
                <motion.div
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {artifact?.verified ? (
                          ...
                          <span className="text-[10px] text-amber-400">Awaiting verification</span>
                        )}
                      </div>
      ...
      {activityFeed.length > 0 ? (
        <div className="shrink-0 border-t border-white/6 px-5 py-3 max-h-[140px] overflow-y-auto ula-scrollbar">
          <p className="text-[10px] uppercase tracking-widest text-zinc-600 mb-2">Recent class activity</p>
          {activityFeed.slice(0, 6).map((e) => (
            <p key={e.id} className="text-[11px] text-zinc-500 py-1 border-b border-white/[0.03] last:border-0">
