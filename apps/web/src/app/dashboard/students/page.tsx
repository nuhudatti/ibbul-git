      ...
      <div className="mx-auto max-w-370">
        ...
        {!loading && !error && filteredStudents.length > 0 ? (
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/20 shadow-2xl shadow-black/20">
            ...
            {filteredStudents.map(({ matric, student }, index) => {
              const studentReviews = reviewMap[normalizeMatric(matric)] ?? [];
              const latestReview = studentReviews.reduce<Review | null>((latest, review) => {
                if (!review.updatedAt) return latest;
                if (!latest) return review;
                return new Date(review.updatedAt).getTime() > new Date(latest.updatedAt ?? 0).getTime() ? review : latest;
              }, null);

              const activeReviews = studentReviews.filter((review) =>
                review.status === "SUBMITTED" || review.status === "UNDER_REVIEW" || review.status === "CHANGES_REQUESTED" || review.status === "RESUBMITTED"
              );

              return (
                <Link
                  key={matric}
                  href={`/dashboard/students/${encodeURIComponent(normalizeMatric(matric))}`}
                  className={cn(
                    "grid w-full gap-3 border-b border-white/8 px-5 py-4 text-left no-underline transition-colors hover:bg-white/4.5 md:grid-cols-[1fr_160px_160px_160px_96px] md:items-center md:gap-4",
                    index === filteredStudents.length - 1 ? "border-b-0" : ""
                  )}
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="truncate font-medium text-white">{student.displayName}</p>
                        <p className="truncate text-xs text-zinc-500">{matric}</p>
                      </div>
                    </div>
                  </div>
                  <div>
                    <p className="text-sm text-zinc-200">{assignments.filter((a) => a.status === "PUBLISHED").length}</p>
                  ...
                  <div>
                    {latestReview ? (
                      <p className="truncate text-sm text-white">{statusLabel[latestReview.status]}</p>
                    ) : (
                      <p className="text-sm text-zinc-400">No review</p>
                    )}
                    <p className="text-xs text-zinc-500">{formatDate(latestReview?.updatedAt ?? latestReview?.submittedAt)}</p>
                  </div>
                  <div className="flex items-center justify-end text-cyan-300">
