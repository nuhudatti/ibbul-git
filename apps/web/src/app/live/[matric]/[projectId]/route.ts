...
export async function GET(
	...
	const deployUrl = `/live/${normalizedMatric}/${normalizedProjectId}`;

	// Parse nested path segments from the full request URL so this single
	// route can serve both the base project root and nested assets/paths.
	const url = new URL(request.url);
	let rawPath = url.pathname || "/";
	if (rawPath.startsWith(deployUrl)) {
		rawPath = rawPath.slice(deployUrl.length);
	}
	const pathSegments = rawPath.split("/").filter(Boolean);
	const requestPath = normalizeRequestPath(pathSegments.length ? pathSegments : undefined);

	const deployment = (await getDeploymentByPath(deployUrl)) as { files: any[] } | null;

	if (deployment) {
		const response = buildLiveDeployResponse(deployment.files as ProjectFile[], requestPath, deployUrl);
		const bodyInit = response.body instanceof Uint8Array ? response.body.buffer : response.body;
		return new NextResponse(bodyInit as any, {
...
// Reuse helper snippets from the previous implementation
