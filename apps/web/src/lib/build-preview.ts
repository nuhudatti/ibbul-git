...
export function buildPreviewHtml(files: ProjectFile[]): string {
  ...
  const runtime = `(() => {
    ...
    const normalizePath = ${normalizePath.toString()};
    const dirname = ${dirname.toString()};
    const normalizeHref = ${normalizeHref.toString()};
    const resolvePreviewUrl = ${resolvePreviewUrl.toString()};
    const isExternalUrl = ${isExternalUrl.toString()};
    const createDataUrl = ${createDataUrl.toString()};
    const rewriteCssUrls = ${rewriteCssUrls.toString()};
