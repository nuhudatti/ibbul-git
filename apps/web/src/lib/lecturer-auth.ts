import { NextResponse } from "next/server";
import { getSession, getSessionTokenFromRequest } from "@/lib/auth-session";
import { getProfileRecordByMatric } from "@/lib/services/student-profile-service";

export async function requireLecturer(req: Request) {
  const token = getSessionTokenFromRequest(req);
  const session = getSession(token);
  if (!session || session.role !== "LECTURER") {
    return {
      error: NextResponse.json(
        { error: "Lecturer authentication required. Please sign out and log in again." },
        { status: 401 }
      ),
    };
  }

  const record = await getProfileRecordByMatric(session.matric);
  if (!record || record.accountRole !== "LECTURER") {
    return {
      error: NextResponse.json({ error: "Invalid lecturer session" }, { status: 401 }),
    };
  }

  return { session };
}
